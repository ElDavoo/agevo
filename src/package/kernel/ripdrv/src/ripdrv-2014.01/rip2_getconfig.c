...
int open_and_map_eRIP_file( char *ripfilename, uint8_t **map ){
	...
	if (infd == -1) {
		...
		if (inst.st_size != RIP2_SZ) {
			fprintf(stderr, "Error: Input file does not match size requirement (%d bytes) for RIP2, aborting!\n", RIP2_SZ);
		} else {
			/* mmap the output file so we can treat it as memory */
			*map = mmap(NULL, RIP2_SZ, PROT_READ, MAP_PRIVATE, infd, 0);
			if (MAP_FAILED == *map) {
...
int main( int argc, char *argv[]){
	...
	if( open_and_map_eRIP_file( ripv2_filename, &ripbuf ) < 0 ) {
		exit(1);
	}

	rip2_flash_init(ripbuf, RIP2_SZ);

	if (rip2_init(0, 1, RIP2_SZ) < 0) {
