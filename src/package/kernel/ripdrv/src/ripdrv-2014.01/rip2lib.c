                                      ...
                                      T_RIP2_ID id)
{
    ...
    /* iterate over index items, check flags, return if match */
    while (from < (T_RIP2_ITEM *)(ripStart + RIP2_OFFSET + RIP2_SZ - sizeof(T_RIP2_HDR))) {
        if ((BETOH32(buf.attr[ATTR_HI]) & RIP2_ATTR_VALID) &&
            (BETOH16(buf.ID) == id)) {
...
#if defined(BUILDTYPE_bootloader) && defined(CONFIG_RIPDRV_CRYPTO_SUPPORT)
